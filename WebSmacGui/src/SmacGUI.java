...
public class SmacGUI extends JFrame implements Runnable, WindowListener, ActionListener, SmacUI
{
	...
	public synchronized void start(boolean generate, boolean export)
	{
		...
		//DB setup variables
		DB_INFO = useConfigFileCheckBox.isSelected()? null : new String[]{mysqlServerTextField.getText(), 
			mysqlDBTextField.getText(), mysqlUserTextField.getText(), mysqlPasswordField.getPassword().toString()};			
	...
	{		
		...
		if(exportToDB)
		{
			...
			outputMessage("Create without adding questions to the DB.\n");
			for (Map<Tag, String> q : allQuestions)
			{
	...
	{				
		try
		{
			...
			outputMessage("\rquestion id " + qid);
	...
	private int findMatchAndExportInDB(Map<Tag, String> q) {
		...
		try
		{
			...
			outputMessage("SmacGUI - Done exporting file to DB.\n");
		}		
